...
const TutorProfile = () => {
  ...
  const fetchUserProfile = async (userId) => {
    try {
      ...
      if (!userSnap.exists()) {
        ...
        if (user) {  // Make sure we have user data from auth
  ...
  const fetchTutorRatings = async (userId) => {
    ...
    try {
      ...
      if (querySnapshot.empty) {
        setRatings([]); // Set an empty array if no ratings exist
        setStatus({
          type: 'info',
  ...
  const handleEducationalEmailSubmit = async (e) => {
    ...
    try {
      ...
      // Update only the tutor-specific fields
      await updateDoc(userRef, {
        ...userData,
        educationalEmail,
        isTutor: true,
      ...
      localStorage.setItem('educationalEmail', educationalEmail);
  ...
  const handleSubjectSubmit = async (e) => {
    ...
    try {
      ...
      const subjectRef = doc(db, 'subjects', subjectForm.subject);
      ...
      setStatus({ type: 'error', message: error.message || 'Failed to add subject' });
    }
  };
          ...
          >
            <div className="bg-white p-6 rounded-xl shadow-sm">
              ...
              <form onSubmit={handleSubjectSubmit} className="space-y-4">
                <div>
                  <label className="block text-sm font-medium mb-1">Subject (Required)</label>
                  <select
